...
public class Pivot
{
   ...
   .OrderBy(x => x);
    ...
    public DataTable PivotData(string rowField, string dataField, AggregateFunction aggregate, bool showSubTotal, IEnumerable<string> columnFields)
    {
        DataTable dt = new DataTable();
        string Separator = ".";
        List<string> rowList = _Source.Select(x => x[rowField].ToString()).Distinct().ToList();
   ...
   .OrderBy(x => x);
        ...
        //var results = columnsLists.CartesianProduct()
        //    .Select(x => string.Join(".", x.Cast<object>()))
        //                        .Distinct()
        //                        .ToList();
        //List<string> colList = _Source.Select(x => columnFields.Aggregate((a, b) => x[a].ToString() + Separator + x[b].ToString())).Distinct().OrderBy(m => m).ToList();

        //if (showSubTotal && columnFields.Count() > 1)
        //{
        //    string totalField = string.Empty;
        //    foreach(var columnField in columnFields)
        //    {
        //        totalField += columnField + "(Total)" + Separator;
        //    }
        //    List<string> totalList = _Source.Select(x => totalField + x[columnFields.Last()].ToString()).Distinct().OrderBy(m => m).ToList();
        //    resultZipped.InsertRange(0, totalList);
        //}

        dt.Columns.Add(rowField);
        resultZipped.ForEach(x => dt.Columns.Add(x));
        ...
        foreach (string rowName in rowList)
        {
            ...
            foreach (string colName in resultZipped)
            {
                ...
                {
                    if (!colField.Contains("(Total)"))
                        filter += " and " + colField + " = '" + colValues[i] + "'";
                    i++;
                }
